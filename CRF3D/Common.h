#ifndef CRF3D_COMMON_H
#define CRF3D_COMMON_H
...
namespace CRF3D {
	...
	//std::string int2str(int nums)
	//{
	//	std::string result;
	//	if (!nums)return "0";
	//	while (nums)
	//	{
	//		result = char('0' + nums % 10) + result;
	//		nums /= 10;
	//	}
	//	return result;
	//}
	...
	inline size_t tokenize2(char *str, const char *del,
		Iterator out, size_t maxNum) {
		char *stre = str + std::strlen(str);///the end place of str
		const char *dele = del + std::strlen(del);///the end place of del
		size_t size = 0;
